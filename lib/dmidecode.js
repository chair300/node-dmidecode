...
var item = function(data,callback) {
  exec('export LC_ALL=C; dmidecode -s ' + data +' unset LC_ALL', function(error, stdout) {
    if (!error) {
...
var toObject = function(data) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (tabs[1] == undefined && tabs[2] == undefined && tabs[0] != undefined) {
      ...
      if (lines[i].startsWith('Handle 0x0') || lines[i].startsWith('End Of Table') || lines[i] == '') {
        continue;
      }
      // we have a root tie
      handle = lines[i];
      ...
      if (keyval[0] != '') {
        ...
        if (keyval[1] == '') {
          ...
          if (isNaN(keyval[1])) { // let's check if our we have a numberic
            obj[handle][keyval[0]] = keyval[1].trim();
          } else {
            obj[handle][keyval[0]] = Number(keyval[1]);
          }
        }
    ...
    if (tabs[0] == '' && tabs[1] == '' && tabs[2] != undefined) {
      ...
      if (obj[handle][secondaryKey] == undefined) {
        obj[handle][secondaryKey] = [];
      }
      //obj[handle][secondaryKey]=[];
      obj[handle][secondaryKey].push(tabs[2]);
    }
  }
